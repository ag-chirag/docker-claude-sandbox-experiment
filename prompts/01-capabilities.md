...
4. Install `jq` with the system package manager if it is not already installed.
...
7. Run the image as a container named `sandbox-task-api-test`, call its `/health` endpoint from inside the sandbox, capture the response, then stop and remove the container.
